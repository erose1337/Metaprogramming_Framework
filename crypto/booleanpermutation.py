...
def byte_transposition(a, b, c, d):                 
...
def permutation(state, rounds=1):
    (a, b, c, d,
     e, f, g, h) = bytes_to_longs(state)
    for round in range(rounds):   
        a, b, c, d = nonlinear_mixing(*bit_transposition(*byte_transposition(a, b, c, d)))
        e, f, g, h = nonlinear_mixing(*bit_transposition(*byte_transposition(e, f, g, h)))
                                                               
        a, b, e, f = nonlinear_mixing(*bit_transposition(*byte_transposition(a, b, e, f)))
        c, d, g, h = nonlinear_mixing(*bit_transposition(*byte_transposition(c, d, g, h)))
                                                               
        a, b, g, h = nonlinear_mixing(*bit_transposition(*byte_transposition(a, b, g, h)))
        c, d, a, b = nonlinear_mixing(*bit_transposition(*byte_transposition(c, d, a, b)))
        
    state[:] = longs_to_bytes(a, b, c, d, e, f, g, h)
    
def composed_bit_byte_transposition(a, b, c, d):  
...
def test_bit_byte_transposition():
    a, b, c, d = 255, 127, 16, 1
    def test_function(a, b, c, d):                                      
        return bit_transposition(*byte_transposition(a, b, c, d))
        
    output1 = test_function(a, b, c, d)
    output2 = composed_bit_byte_transposition(a, b, c, d)
    def _state(words):        
...
def test_permutation():
    from metrics import test_permutation
    def _permutation(_input):
        state = bytearray(_input)
        permutation(state)
        return bytes(state)
        
    test_permutation(_permutation, 32, avalanche_test=False, randomness_test=False)                
        
        
if __name__ == "__main__":
    test_bit_byte_transposition()    
    test_permutation()
