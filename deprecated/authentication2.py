...
#        self.peer_ip[session_id] = ip
    ...
    def register(self, username=''):
        ...
        authentication_table = pride.functions.security.random_bytes(self.authentication_table_size)
    ...
    def login(self, challenge):
        ...
        try:
            ...
            response = pride.functions.security.random_bytes(32)
        else:
    ...
    def login_stage_two(self, hashed_answer, original_challenge):
        ...
        encrypted_key = pride.functions.security.random_bytes(self.shared_key_size + 48 + 32)
        try:
            (saved_table,
             session_key,
            ...
            if authentication_table_hash in self.session_id:
                ...
                new_key = pride.functions.security.random_bytes(self.shared_key_size)
                
                hkdf = invoke("pride.functions.security.hkdf_expand", self.hash_function,
                ...
                login_packet = pride.functions.utilities.save_data(new_key, login_message)                                                    
                encrypted_key = pride.functions.security.encrypt(login_packet, session_key, session_key)                                            
            else:                
...
class Authenticated_Client(pride.objectlibrary.base.Base):   
    ...
    @pride.functions.decorators.with_arguments_from(_supply_username)
    ...
    @pride.functions.decorators.on_exception(IOError, _token_not_registered)
    @pride.functions.decorators.with_arguments_from(_get_auth_table_hash)
    ...
    @pride.functions.decorators.with_arguments_from(_answer_challenge)
    ...
    def decrypt_new_secret(self, encrypted_key):
        with self.create(self.token_file_type, self.authentication_table_file, 
            ...
            try:
                packed_key_and_message = pride.functions.security.decrypt(encrypted_key, shared_key, shared_key)
            except ValueError:                
                self.alert("Login attempt failed", level=self.verbosity["login_failed"])
                self._logging_in = False
                self.handle_login_failure()
            else:
                new_key, login_message = pride.functions.utilities.load_data(packed_key_and_message)
