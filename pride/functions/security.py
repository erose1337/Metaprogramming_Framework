...
from pride.functions.utilities import dumps, loads
...
try:
    ...
    def apply_mac(key, data, algorithm="SHA256", backend=BACKEND):
        """ Generates a message authentication code and prepends it to data.
            Mac and data are packed via pride.functions.utilities.dumps.
    ...
    def encrypt(data='', key='', mac_key='', iv=None, extra_data='', algorithm="AES",
        ...
        if return_mode == "cryptogram":
            return dumps(header, ciphertext, iv, mac_tag, extra_data)
        else:
    ...
    def decrypt(packed_encrypted_data, key, mac_key='', backend=BACKEND):
        ...
        header, ciphertext, iv, tag, extra_data = loads(packed_encrypted_data)
        ...
        if mode in AEAD_MODES:
            ...
            if not verify_mac(mac_key, dumps(tag, header + ciphertext + iv + extra_data), authentication_algorithm):
    ...
    def hash_password(password, iterations, algorithm="pbkdf2hmac", sub_algorithm="sha256",
        ...
        if algorithm == "pbkdf2hmac":
            ...
            return dumps(header, salt, hasher.derive(header + password))
        else:
            raise ValueError()
