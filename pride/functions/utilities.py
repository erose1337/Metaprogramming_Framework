...
def resolve_string(module_name):
    ...
    if module_name[0] == "/":
        try:
            ...
            result = globals()["pride"].objects[module_name]
        else:
            result = getattr(globals()["pride"].objects[reference], attribute)
    elif module_name in dir(__builtins__):
        result = getattr(__builtins__, module_name)
    else:
        result = None
