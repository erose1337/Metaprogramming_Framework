...
# pride.objectlibrary.shell.Python_Shell -> pride.objectlibrary.interpreter.Shell -> /Python/Interpreter -> .../Python_Shell/Prompt   
...
class Python_Shell(pride.gui.gui.Application):
    ...
    def __init__(self, **kwargs):
        ...
        shell_connection = self.create("pride.objectlibrary.interpreter.Shell", username=self.username, 
                                       startup_definitions=self.startup_definitions,
                                       ip=self.ip, port=self.port, target_service=self.target_service,
                                       stdout=self)
        shell_interface = self.create("pride.objectlibrary.shell.Python_Shell", shell=shell_connection.reference)        
