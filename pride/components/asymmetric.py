...
class RSA_Private_Key(pride.base.Wrapper):
    ...
    def __init__(self, **kwargs):
        ...
        if self.keyfile:
            ...
            private_key = load_pem_private_key(encoded_key, raw_input(prompt) or None, pride.functions.security.BACKEND)
        else:
            private_key = rsa.generate_private_key(self.public_exponent, self.keysize, pride.functions.security.BACKEND)
...
class EC_Private_Key(pride.base.Wrapper):
    ...
    def __init__(self, **kwargs):
        ...
        private_key = ec.generate_private_key(getattr(ec, self.curve_name), pride.functions.security.BACKEND)
        self.wraps(private_key)
    ...
    def __getstate__(self):
        state = super(EC_Private_Key, self).__getstate__()
        state["key"] = self.serialize()
        del state["wrapped_object"]
        return state

    def on_load(self, state):
        super(EC_Private_Key, self).on_load(state)
        self.wraps(load_pem_private_key(state["key"], password=None, backend=pride.functions.security.BACKEND))
...
class EC_Public_Key(pride.base.Wrapper):
    ...
    def __getstate__(self):
        state = super(EC_Public_Key, self).__getstate__()
        state["key"] = self.serialize()
        del state["wrapped_object"]
        return state

    def on_load(self, state):
        super(EC_Public_Key, self).on_load(state)
        self.wraps(load_pem_public_key(state["key"], backend=pride.functions.security.BACKEND))
